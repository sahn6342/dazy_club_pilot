...
| DEC-035 | Café pre-order attached to a booking via `orders.booking_id` (soft link, nullable); public `POST /bookings/{ref}/preorder` reuses the same identity model as booking creation (ref + matching contact, no login), gated to confirmed bookings only; reuses `pos_service.create_order` so pricing/tax/totals logic isn't duplicated | Accepted — implemented (Detailed-Roadmap Phase 7, sub-step 1 of café × turf synergy); wallet is sub-step 2, not yet built |
| DEC-036 | Self-service booking lookup/resume (`GET /bookings/lookup?ref=&contact=`, no login) — the stored `checkoutJson` from booking creation is replayed verbatim on resume, never a second payment-gateway order for one booking; a payment-pending reminder notification fires once at booking creation with the resume link | Accepted — implemented; fixes a real UX bug found in manual Razorpay testing (accidentally clicking another slot/tab/date pill while a payment was pending silently abandoned it with no way back, since there's no login/profile system) |
